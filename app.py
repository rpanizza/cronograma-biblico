...
# Versão do Aplicativo (App) - Correção final do TypeError no st.expander do Painel de Controle
VERSAO_APP = "1.9.2" 
# Versão do Conteúdo (Cronologia)
VERSAO_CONTEUDO = "25.1208.21" 
...
def run_ia_search(prompt):
    ...
    with st.spinner("Consultando IA e formatando dados..."):
        ...
        st.session_state['ia_raw_result'] = raw_text
...
if 'ia_response_text' not in st.session_state: st.session_state['ia_response_text'] = None
if 'ia_raw_result' not in st.session_state: st.session_state['ia_raw_result'] = ""
if 'show_ia_preview' not in st.session_state: st.session_state['show_ia_preview'] = False
if 'control_panel_expanded' not in st.session_state: st.session_state['control_panel_expanded'] = False
...
if admin_mode:
    ...
    # 1. Defina o estado de expansão ANTES de criar o expander:
    # Se estiver editando, force a expansão. Caso contrário, use o estado salvo na sessão.
    ...
    with st.expander("🛠️ Painel de Controle (Edição Completa)", expanded=control_panel_expanded, key='control_panel_expander'):
        ...
        # 3. Use o valor da chave para controlar o estado persistido:
        # Quando o usuário clicar no expander, o valor da chave será a fonte da verdade para o estado.
        ...
        # --- FORMULÁRIO DE ADIÇÃO/EDIÇÃO MANUAL ---
